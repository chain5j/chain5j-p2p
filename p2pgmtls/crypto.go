...
func NewIdentity(cert *gmtls.Certificate, caRoots [][]byte) (*Identity, error) {
	// CA证书池
	...
	return &Identity{
		config: &gmtls.Config{
			...
			Certificates:       []gmtls.Certificate{*cert}, // 自己的证书链
...
func (i *Identity) ConfigForAny() (*gmtls.Config, <-chan ci.PubKey) {
	return i.ConfigForPeer("")
}

func (i *Identity) ConfigForPeer(remote peer.ID) (*gmtls.Config, <-chan ci.PubKey) {
